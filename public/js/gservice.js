...
angular.module('gservice', []).factory('gservice', function($rootScope, $http){
        ...
        var convertToMapPoints = function(response){
            ...
            for(var i= 0; i < response.length; i++) {
                ...
                var  contentString =
                    '<p><b>Store Name</b>: ' + store.name +
                    '<br><b>City</b>: ' + store.city +
                    '<br><b>Country</b>: ' + store.country +
                    '<br><b>Total Visitors</b>: ' + store.total_visitors +
                    '</p>';

                // Converts each of the JSON records into Google Maps Location format (Note [Lat, Lng] format).
                locations.push({
                    latlon: new google.maps.LatLng(store.latitude, store.longitude),
